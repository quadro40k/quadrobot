...
module.exports = {
	name: 'ptgracer',
	description: 'Outputs current PTG Racing team roster',
    aliases: ['racers', 'ptgracers'],
	cooldown: 5,
	execute(message) {
        ...
        sheets.spreadsheets.values.get({
            ...
            if (rows.length) {
                ...
                rows.forEach(player => {
                    if(player[racer] == "Yes")
                    outputArray.push(player[name].slice(0,16).padEnd(16," ") +" " + player[location]);
                ...
                message.channel.send(`\`\`\`PTG Racing Team:\n${outputArray.join("\n")}\`\`\``);
